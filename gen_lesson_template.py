...
def render_lesson(d):
  ...
  <section class="section" id="sec-reading">
    ...
    <div class="card">
      <button class="pill" id="gram-toggle">🔍 Show grammar in this text</button>
      <div class="passage" id="passage" style="margin-top:14px">{passage_paragraphs}</div>
      <p style="font-size:12.5px;color:var(--text-tertiary);margin-top:10px">Tap a highlighted word for pronunciation, meaning and an example. With grammar highlighting on, tap a highlighted phrase to see why that form was used.</p>
    </div>
    <div class="card">
  ...
  <section class="section" id="sec-vocab">
    ...
    <div class="card">
      <p style="font-size:14.5px;margin-bottom:16px">All nine words from the text — mixed formats.</p>
  ...
  <section class="section" id="sec-practice">
    ...
    <div class="card">
      ...
      <p style="font-size:12.5px;text-transform:uppercase;letter-spacing:.03em;color:var(--text-tertiary);margin-bottom:16px">Grammar focus — {practice["gapfill_focus"]}</p>
      {practice_gap_html}
    </div>
    {second_html}
    <div class="card">
  ...
  Course.renderCategorise('cat-root', {j(second_widget["items"])}, {j(second_widget["categories"])}, {{lessonId: LESSON_ID, sectionPrefix: 'practice-cat'}});'''
    ...
    words: {j(item["words"])}, errorIndices: {j(item["error_indices"])}, correction: {j(item["correction"])}
  }});'''
            )
        second_js = "\n".join(parts)
